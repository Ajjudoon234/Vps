...
def approve_group_members(group_id, plan, valid_until):
    try:
        ...
        for member in members:
            ...
            save_user(user_data)
        bot.send_message(group_id, "*All users in this group are approved.*", parse_mode='Markdown')
...
def approve_or_disapprove_user(message):
    ...
    if len(cmd_parts) < 2:
        bot.send_message(chat_id, "*Invalid command format. Use /approve <user_id/group_id> <plan> <days> or /disapprove <user_id/group_id>.*", parse_mode='Markdown')
        return

    action = cmd_parts[0]
    target_id = int(cmd_parts[1])  # This could be a user ID or group ID
    plan = int(cmd_parts[2]) if len(cmd_parts) >= 3 else 0
    days = int(cmd_parts[3]) if len(cmd_parts) >= 4 else 0

    if action == '/approve':
        if plan in [1, 2] and (count_users_with_plan(plan) >= 99 if plan == 1 else count_users_with_plan(plan) >= 499):
            bot.send_message(chat_id, "*Approval failed: Plan limit reached.*", parse_mode='Markdown')
            return

        valid_until = (datetime.now() + timedelta(days=days)).date().isoformat() if days > 0 else datetime.now().date().isoformat()

        # Check if it's a group approval
        try:
            members = bot.get_chat_administrators(target_id)  # Get group members
            for member in members:
                user_data = {
                    "user_id": member.user.id,
                    "plan": plan,
                ...
                save_user(user_data)
            msg_text = f"*Group {target_id} approved with plan {plan} for {days} days.*"
        except ApiTelegramException:
            # If it's not a group, handle individual user approval
            user_data = {
                "user_id": target_id,
                "plan": plan,
                "valid_until": valid_until,
            ...
            save_user(user_data)
            msg_text = f"*User {target_id} approved with plan {plan} for {days} days.*"
        ...
        if user_data:
            ...
            save_user(user_data)
        msg_text = f"*User {target_id} disapproved and reverted to free.*"
